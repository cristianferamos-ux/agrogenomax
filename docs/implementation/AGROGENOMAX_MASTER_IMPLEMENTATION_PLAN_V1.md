...
**Estado vigente**: LOTE-015 (matriz documental de variables de entorno de staging, `server/.env.staging.example`) está COMPLETADO, commit `9b2d11e`. LOTE-016 (código del bootstrap de estado remoto Terraform de staging, sin `apply`) queda DEFINIDO — PENDIENTE DE APROBACIÓN/EJECUCIÓN (definición completa en la sección dedicada tras LOTE-015). Definir LOTE-016 no ejecuta Terraform ni crea ningún recurso AWS. Esto no significa que Fase 4 haya iniciado: Fase 4 de infraestructura todavía NO ha iniciado; no se ha ejecutado ningún `terraform apply` ni se ha creado ningún recurso AWS. Los lotes adicionales de infraestructura de esta fase (§14, párrafo siguiente) siguen pendientes y cada uno requiere definición completa y aprobación individual antes de ejecutarse.

**Lotes adicionales de esta fase, no detallados en los primeros 15** (registrados como pendientes, §24, cada uno requerirá su propia definición completa de lote antes de ejecutarse): **LOTE-016 — código del bootstrap de estado remoto Terraform de staging + *locking* nativo S3 (alcance: código y validación local solamente, sin aprovisionamiento; definición detallada más abajo, DEFINIDO — PENDIENTE DE APROBACIÓN)**; módulo de rol OIDC de CI/CD; módulo ECR; módulo ECS Express Mode + ALB de staging; módulo RDS `agx-staging`; módulo/instancia `postgis-catastrox-staging` (plano separado); módulo Secrets Manager de staging; configuración de CloudWatch de staging; configuración de dominio `staging.agrogenomax.com` en Cloudflare; validación del gate de staging ya exigido por ADR-011 §14.1/§19.1.
...
| ADR-008 | Fase 6 | — |
| ADR-009 | Fase 5 | — |
| ADR-010 | Fase 4 | LOTE-016 (código de bootstrap definido, PENDIENTE DE APROBACIÓN/EJECUCIÓN — no completado) |
...
| LOTE-016 | DEFINIDO — PENDIENTE DE APROBACIÓN | — | Bootstrap Terraform de staging (S3 + CMK dedicada + *locking* nativo S3 vía `use_lockfile=true`, sin DynamoDB); alcance limitado a código y validación local (`terraform fmt`/`init -backend=false`/`validate`); sin `apply`, sin conexión AWS, sin recursos creados. Definición completa en la sección dedicada tras LOTE-015. |
...
- Diseño lote por lote de las Fases 4 a 10 más allá del primer lote de Fase 4 (Lote 15) — se detallarán cuando corresponda avanzar a ellas. LOTE-016 (código del bootstrap de estado remoto Terraform de staging, sin `apply`) es la única excepción ahora definida en esta edición documental; su implementación real permanece fuera de alcance de esta edición, así como cualquier lote adicional aún no definido.
...
### LOTE-016 — Código del bootstrap de estado remoto Terraform de staging (sin `apply`)

| Campo | Valor |
|---|---|
| ID | LOTE-016 |
| Fase | Fase 4, preparación técnica; no inicia aprovisionamiento |
| ADR que lo autoriza | ADR-010, especialmente §§4, 8-15, 18-19, 22 y 25-29 |
| Estado | Definido — pendiente de aprobación |
| Estado vigente | Aún no ejecutado; cero Terraform/AWS |
| Objetivo | Crear código reproducible y validable localmente para el bootstrap de staging que, solo en un lote operativo posterior, pueda crear el bucket S3 del *state*, la CMK dedicada y la configuración necesaria para *locking* nativo S3; el código también debe definir las políticas IAM mínimas de staging necesarias para los futuros actores de *plan* y *apply*; no se crean todavía roles OIDC ni se asocia ninguna identidad real |
| Precondiciones | LOTE-014 y LOTE-015 completados; ADR-010 vigente; inventario confirma ausencia actual de Terraform; aprobación explícita del usuario antes de ejecutar LOTE-016 |
| Archivos permitidos | `.gitignore`; nuevos archivos bajo `infra/terraform/bootstrap/staging/`; archivo de versión de Terraform dentro del árbol `infra/terraform/`; `.terraform.lock.hcl` del *root module*, generado por Terraform y versionado; un *runbook* nuevo y acotado bajo `docs/runbooks/` para validación local y futura migración del *state* |
| Archivos prohibidos | Código de aplicación `server/`, `src/`, `functions/` y `shared/`; ADR existentes; *workflows* `.github`; `Dockerfile`/`.dockerignore`; `server/sql` y activos no rastreados preexistentes; cualquier `.env`, *tfvars* real, *state*, *backup*, plan binario o credencial |
| Cambios exactos | Endurecer `.gitignore` para excluir `.terraform/`, `*.tfstate`, `*.tfstate.*`, *crash logs*, `*.tfplan`, *tfvars* sensibles y archivos *backend* locales generados, pero permitir/versionar `.terraform.lock.hcl` y archivos `.tf` legítimos; fijar una versión exacta estable de Terraform, elegida y documentada durante la ejecución con evidencia oficial y verificada como compatible con `use_lockfile`; declarar *provider* AWS con restricción de versión y generar/versionar `.terraform.lock.hcl`; crear *root module* de bootstrap de staging con recursos/configuración para: bucket S3 exclusivo de *state* de staging/compartido; bloqueo de acceso público; *Bucket owner enforced*; versionado; cifrado SSE-KMS con CMK dedicada; rotación automática de CMK; S3 Bucket Key cuando corresponda; política TLS obligatorio; protección coherente contra eliminación accidental; etiquetas de ambiente/proyecto/gestión/costo; documentar el *backend* futuro con `use_lockfile=true` y separación entre `terraform.tfstate` y `.tflock`; resolver explícitamente el ciclo bootstrap local temporal → migración futura al *backend* remoto mediante un procedimiento reproducible, sin ejecutar dicha migración en LOTE-016; no crear DynamoDB; no crear OIDC, *workflows*, ECR, ECS, RDS, CloudTrail ni otros recursos de Fase 4; no incluir nombres/ARN/*account id* secretos o reales no aprobados, usar variables/convenciones validadas; codificar en el *root module* políticas IAM de mínimo privilegio para uso posterior, con reglas obligatorias: política de *plan* — puede listar únicamente el prefijo autorizado, puede leer `terraform.tfstate`, nunca puede escribir ni eliminar `terraform.tfstate`, puede leer, crear y eliminar únicamente el objeto `<state-key>.tflock`; política de *apply* — puede leer y escribir `terraform.tfstate`, no necesita ni recibe `DeleteObject` sobre `terraform.tfstate`, puede leer, crear y eliminar únicamente el objeto `<state-key>.tflock`; ambas políticas reciben solo uso criptográfico de la CMK específica de staging (`Decrypt`, `GenerateDataKey`, `Encrypt` cuando corresponda y `DescribeKey`) y quedan restringidas a la CMK exacta, uso vía S3 y recursos/prefijos autorizados; ninguna política recibe administración KMS (`PutKeyPolicy`, `DisableKey`, `ScheduleKeyDeletion`, `EnableKeyRotation`) ni administración no controlada de *grants*; estas políticas pueden quedar definidas como recursos/*policy documents* reutilizables por el lote OIDC posterior, pero LOTE-016 no crea proveedor OIDC, *trust policy* de GitHub, *workflow* ni rol asumible por GitHub |
| Pruebas/validaciones autorizadas (futura ejecución) | `terraform version`; `terraform fmt -check -recursive`; `terraform init -backend=false`; `terraform validate`; generación y revisión de `.terraform.lock.hcl`; `terraform providers lock` para `windows_amd64` y `linux_amd64` si técnicamente corresponde; escaneo estático local sin credenciales; inspección negativa de secretos y artefactos prohibidos; `git diff --check`; ninguna operación que consulte AWS y ningún plan/*apply*; inspección estática negativa/positiva de las políticas IAM: política de *plan* sin `PutObject`/`DeleteObject` sobre `terraform.tfstate`, política de *apply* sin `DeleteObject` sobre `terraform.tfstate`, ambas con operaciones completas únicamente sobre `.tflock`, ninguna con permisos administrativos KMS, y restricciones a CMK/prefijo/vía S3 verificadas |
| Criterio de aceptación | Código válido y reproducible localmente; `use_lockfile=true` documentado para el *backend* futuro; sin DynamoDB; *state*/planes/*tfvars* sensibles ignorados; `.terraform.lock.hcl` versionado; cero credenciales; cero recursos AWS; *runbook* claro para que un lote operativo posterior solicite aprobación antes del *apply* y migración; las políticas IAM mínimas cumplen exactamente la separación plan/apply/*state*/*lock*/KMS descrita y quedan listas para consumo por un lote OIDC posterior, sin crear identidades reales |
| Evidencia requerida | Versiones fijadas; salidas de `fmt`/`init -backend=false`/`validate`/escaneo; listado exacto de archivos; revisión de secretos; estimación preoperativa de costos separando CMK, S3 y auditoría futura |
| Costos | LOTE-016 no genera costo AWS porque no conecta ni aprovisiona; antes del lote operativo posterior se debe recalcular en AWS Pricing Calculator; el estimado debe permanecer dentro del presupuesto total de staging de USD 25/mes; no afirmar costo exacto futuro como inmutable |
| Rollback | `git revert` del commit de LOTE-016 para el código; no hay *rollback* AWS porque este lote no crea recursos |
| Riesgo | Medio, por tratarse del primer código IaC y por el carácter sensible del *state* |
| Tamaño | M |
| Dependencias | ADR-010; LOTE-014; LOTE-015 |
| Tareas paralelizables | Ninguna durante la definición del bootstrap base |
| Estrategia Git | Rama `lote/016-terraform-remote-state-code`; *staging* selectivo; un *commit* aislado; sin *push* sin autorización |
| Commit esperado | Uno |
| Mensaje recomendado | `build(terraform): add staging remote-state bootstrap code` |
| Fuera de alcance | `terraform plan/apply/destroy/import/state/force-unlock`; cualquier llamada o autenticación AWS; migración real del *state*; creación real de *bucket*/CMK/IAM; OIDC y GitHub Actions; producción; cualquier recurso distinto del *backend* de *state* definido; modificación funcional de AgroGenomaX |
| Gate posterior | LOTE-016 completado y validado no autoriza aprovisionar; deberá definirse y aprobarse otro lote operativo para credenciales administrativas temporales, revisión final de costo, *apply* del bootstrap, verificación del *bucket*/CMK/*locking* y migración del *state* local temporal conforme a ADR-010 §14.3 |
...
15 lotes iniciales detallados: 12 lotes completados, 1 lote parcial (LOTE-003), 2 con auditoría/verificación específica (LOTE-011 y LOTE-012) y 0 pendientes (referido exclusivamente a estos 15 lotes iniciales), con las Fases 5-10 registradas a nivel de fase y pendientes de descomposición lote por lote en tareas futuras (§24). Adicionalmente, 1 lote adicional definido: LOTE-016 (bootstrap Terraform de staging, código sin `apply`), pendiente de aprobación/ejecución.
...
LOTE-013 completado (commit `cf99371`). Los 15 lotes iniciales quedan sin pendientes: 12 completados, 1 parcial (LOTE-003) y 2 con auditoría/verificación específica (LOTE-011 y LOTE-012). LOTE-016 (código del bootstrap de estado remoto Terraform de staging, sin `apply`) queda ahora DEFINIDO — PENDIENTE DE APROBACIÓN/EJECUCIÓN (definición completa tras LOTE-015). Siguiente paso recomendado: ejecutar LOTE-016 **solo después de aprobación explícita** del usuario. El siguiente paso no es `terraform apply`: Fase 4 de infraestructura sigue sin iniciar.

Justificación: con LOTE-013 cerrado no queda ningún lote de los primeros 15 en estado pendiente; definir LOTE-016 tampoco autoriza ejecutar `terraform apply` ni crear ningún recurso AWS — se requiere aprobación explícita para ejecutar el propio LOTE-016, y posteriormente la definición y aprobación de un lote operativo separado (revisión de costos/presupuesto, credenciales temporales y *rollback*) antes de cualquier aprovisionamiento real.

### 6. Dependencias

Documentadas en §5 (ADR→ADR) y en el campo "Dependencias" de cada lote (§ definición detallada). LOTE-015 y LOTE-013 quedan retirados como pendientes (LOTE-015 completado, commit `9b2d11e`; LOTE-013 completado, commit `cf99371`). LOTE-016 (bootstrap Terraform de staging, código sin `apply`) queda DEFINIDO — PENDIENTE DE APROBACIÓN/EJECUCIÓN; definirlo no lo autoriza a ejecutarse, ni autoriza `terraform apply` ni recursos AWS. Cadena de dependencias vigente: aprobación y ejecución local de LOTE-016 → definición/aprobación del lote operativo de bootstrap (credenciales temporales, costo, *apply*, verificación y migración del *state*) → eventual aprovisionamiento de staging. Ningún otro lote adicional de Fase 4 (más allá de LOTE-015 y LOTE-016) está definido o autorizado todavía. LOTE-003 conserva remanentes no bloqueantes para *readiness*, pero obligatorios antes de cerrar integralmente ADR-014. LOTE-011 sigue requiriendo auditoría final y LOTE-012 sigue pendiente de verificación.
