...
// CatastroX. Corrige el hallazgo de la auditoría -- reflejaba `Origin` sin
// validar, con fallback `*`, reflejaba `Access-Control-Request-Headers`
// sin filtrar, y caía a una URL de Railway hardcodeada si `API_BACKEND_URL`
// no estaba configurada. Ahora usa la misma política CORS pura que el
// backend Express (shared/security/corsPolicy.js) y nunca elige un
// upstream sugerido por el cliente.
import {
...
export async function onRequest(context) {
  ...
  if (!ALLOWED_RELAY_ENVIRONMENTS.includes(appEnv)) {
    console.warn(`[AgroGenomaX relay] APP_ENV ausente o inválida -- relay bloqueado.`);
  ...
  try {
    ...
    console.warn(`[AgroGenomaX relay] Allowlist de CORS inválida para ambiente="${appEnv}" -- relay bloqueado.`);
    return configErrorResponse();
  }
  ...
  if (decision.action === 'reject') {
    console.warn(
      `[AgroGenomaX relay] Origen rechazado (ambiente="${appEnv}", motivo="${decision.reason}", metodo="${request.method}").`,
    );
  ...
  if (!backendBaseUrl) {
    console.warn(`[AgroGenomaX relay] API_BACKEND_URL ausente o inválida para ambiente="${appEnv}".`);
    return configErrorResponse();
  }
