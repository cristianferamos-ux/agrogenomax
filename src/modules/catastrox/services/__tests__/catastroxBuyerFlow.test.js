...
// de customerId antes de intentar /checkout, la lista cerrada de campos
// enviados al backend, ausencia de PII en localStorage, y las utilidades
// puras de enmascarado/estado honesto. Fixtures exclusivamente sintéticos;
// fetch siempre se simula, nunca se contacta un backend real.
...
test('createCustomer) POST a /customers con credentials:include, devuelve customerId + devOtpCode tal cual el backend', async () => {
  let requestedUrl = null;
  let requestedOptions = null;
  globalThis.fetch = async (url, options) => {
    requestedUrl = String(url);
    requestedOptions = options;
    return jsonResponse({ ok: true, customerId: 'cust-1', emailVerificationRequired: true, devOtpCode: '123456' });
  };
...
test('createCustomer) nunca fabrica devOtpCode si el backend no lo envía (producción)', async () => {
  globalThis.fetch = async () => jsonResponse({ ok: true, customerId: 'cust-2', emailVerificationRequired: true });
...
test('verifyCustomerEmail) POST a /customers/:id/verify-email con credentials:include, el código viaja solo en el body', async () => {
  let requestedUrl = null;
  let requestedOptions = null;
  globalThis.fetch = async (url, options) => {
    requestedUrl = String(url);
  ...
  const result = await paymentService.verifyCustomerEmail({ customerId: 'cust-1', code: '654321' });
  assert.equal(result.ok, true);
  assert.equal(result.verified, true);
  assert.match(requestedUrl, /\/customers\/cust-1\/verify-email$/);
  assert.equal(requestedOptions.credentials, 'include');
  assert.equal(JSON.parse(requestedOptions.body).code, '654321');
});
...
test('verifyCustomerEmail) código incorrecto -> ok:false con el code del backend (CODE_MISMATCH)', async () => {
  ...
  const result = await paymentService.verifyCustomerEmail({ customerId: 'cust-1', code: '000000' });
  assert.equal(result.ok, false);
  assert.equal(result.code, 'CODE_MISMATCH');
});

test('verifyCustomerEmail) sin customerId o sin code -> no llama a la red', async () => {
  globalThis.fetch = async () => {
    throw new Error('No debe llamarse a la red sin customerId/code.');
  };

  const result = await paymentService.verifyCustomerEmail({ customerId: '', code: '123456' });
  assert.equal(result.ok, false);
...
test('14) startPackageCheckout SIN customerId nunca llama a la red -- customerId solo se envía después de verificar correo', async () => {
  globalThis.fetch = async (url) => {
    throw new Error(`No debe llamarse a la red sin customerId: ${url}`);
  };
...
test('startPackageCheckout SIN purchaseAttemptId tampoco llama a la red (protección de doble clic exige un intento identificado)', async () => {
  ...
  const result = await paymentService.startPackageCheckout({ packageId: 'basico', lookup, customerId: 'cust-1' });
  assert.equal(result.status, 'error');
});

test('6/18) el body de /checkout contiene EXCLUSIVAMENTE la lista cerrada de campos -- nunca precio/monto/moneda/reference/estado/email/documento', async () => {
  ...
  assert.equal(requestedBody.customerId, 'cust-checkout-1');
...
test('22) createCustomer/verifyCustomerEmail nunca escriben en localStorage', async () => {
  globalThis.fetch = async (url) => {
    if (String(url).includes('/verify-email')) return jsonResponse({ ok: true, verified: true });
    return jsonResponse({ ok: true, customerId: 'cust-storage', emailVerificationRequired: true, devOtpCode: '111111' });
  };
  ...
  await paymentService.verifyCustomerEmail({ customerId: 'cust-storage', code: '111111' });

  assert.equal(localStorageMap.size(), 0, 'ningún dato de comprador/OTP debe persistirse en localStorage');
});
