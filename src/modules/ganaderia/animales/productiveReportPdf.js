...
class PdfBuilder {
  ...
  centeredText(value, centerX, y, size = 10, font = FONT.regular, color = [0.06, 0.09, 0.12]) {
    const text = sanitizePdfText(value);
    const estimatedWidth = text.length * size * 0.27;
    this.text(text, centerX - estimatedWidth, y, size, font, color);
  }
  ...
  header(title, generatedAt) {
    ...
    this.drawWrapped('Sistema Inteligente de Gestión Ganadera, Trazabilidad y Cumplimiento Productivo', MARGIN + 88, this.y - 75, 62, 7.4, FONT.bold, [0.06, 0.09, 0.12], 9);
