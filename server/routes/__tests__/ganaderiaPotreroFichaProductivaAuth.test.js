...
// superficie de /api/ganaderia/predios/:predioId/potreros/:potreroId/ficha-productiva
// exige sesión Ganadería con organización activa, y que las mutaciones
// exigen CSRF. Mismo patrón que ganaderiaPotrerosAuth.test.js -- servidor
// HTTP efímero real, fetch() directo, sin mocks de middleware, sin DB
// configurada.
import { test, describe, before, beforeEach } from 'node:test';
import assert from 'node:assert/strict';
import express from 'express';

import createGanaderiaPotreroFichaProductivaRouter from '../ganaderiaPotreroFichaProductiva.js';
...
function startApp() {
  ...
  app.use(
    ...
    createGanaderiaPotreroFichaProductivaRouter(ROUTER_CONFIG),
  );
  app.use(notFound);
...
describe('SPRINT-3D6: ficha productiva exige sesión con organización', () => {
  test('GET .../ficha-productiva sin sesión -> 401', async () => {
    await assertAnonymousRejected('GET', '/101/potreros/5/ficha-productiva');
  });

  test('POST .../ficha-productiva sin sesión -> 401', async () => {
    await assertAnonymousRejected('POST', '/101/potreros/5/ficha-productiva', {
      especies: [{ pasturaId: '1' }],
  ...
  test('nunca toca Postgres-AGX-Business en una solicitud anónima', async () => {
    const ctx = await startApp();
    try {
      const response = await fetch(`${ctx.baseUrl}/101/potreros/5/ficha-productiva`);
