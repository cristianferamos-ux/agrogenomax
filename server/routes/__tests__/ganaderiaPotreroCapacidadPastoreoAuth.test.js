// SPRINT-3D7-CAPACIDAD-PASTOREO: garantía server-side de que TODA la
// superficie de /api/ganaderia/predios/:predioId/potreros/:potreroId/capacidad-pastoreo
// exige sesión Ganadería con organización activa, y que las mutaciones
// exigen CSRF. Mismo patrón que ganaderiaPotreroFichaProductivaAuth.test.js
...
import createGanaderiaPotreroCapacidadPastoreoRouter from '../ganaderiaPotreroCapacidadPastoreo.js';
...
function startApp() {
  ...
  app.use(
    ...
    createGanaderiaPotreroCapacidadPastoreoRouter(ROUTER_CONFIG),
  );
  app.use(notFound);
...
describe('SPRINT-3D7: capacidad de pastoreo exige sesión con organización', () => {
  test('GET .../capacidad-pastoreo sin sesión -> 401', async () => {
    await assertAnonymousRejected('GET', '/101/potreros/5/capacidad-pastoreo');
  });

  test('POST .../capacidad-pastoreo/preview sin sesión -> 401', async () => {
    await assertAnonymousRejected('POST', '/101/potreros/5/capacidad-pastoreo/preview', VALID_DIAS_BODY);
  });

  test('POST .../capacidad-pastoreo sin sesión -> 401', async () => {
    await assertAnonymousRejected('POST', '/101/potreros/5/capacidad-pastoreo', VALID_DIAS_BODY);
  });

  test('nunca toca Postgres-AGX-Business en una solicitud anónima', async () => {
    const ctx = await startApp();
    try {
      const response = await fetch(`${ctx.baseUrl}/101/potreros/5/capacidad-pastoreo`);
